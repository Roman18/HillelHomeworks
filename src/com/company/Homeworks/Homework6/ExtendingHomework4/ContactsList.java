...
public Contact get(int i){// получение элемента по индексу (для перебора элементов)
    ...
    public ContactsList searchByName(String name){
        ContactsList contactsList=new ContactsList();
        for (int i = 0; i <index ; i++) {
        if (get(i).getName().startsWith(name)){
            contactsList.add(get(i));
        }
        }
        return contactsList;
    }

    public ContactsList searchByNumber(String name){
        ContactsList contactsList=new ContactsList();
        for (int i = 0; i <index ; i++) {
            if (get(i).getPhone().contains(name)){
                contactsList.add(get(i));
            }
        }
        return contactsList;
